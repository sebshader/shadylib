/* ripbob~ - use a differential equation solver to imitate an analogue circuit */
...
static t_class *ripbob_class;

static void ripbob_saturation(t_ripbob *x, t_float saturation)
{
    if (saturation <= 1e-3)
        saturation = 1e-3;
...
static void ripbob_oversample(t_ripbob *x, t_float oversample)
{
    if (oversample <= 1)
        oversample = 1;
...
static void ripbob_clear(t_ripbob *x)
{
    int i;
    for (i = 0; i < DIM; i++)
...
static void ripbob_print(t_ripbob *x)
{
    int i;
    for (i = 0; i < DIM; i++)
...
static void *ripbob_new( void)
{
    t_ripbob *x = (t_ripbob *)pd_new(ripbob_class);
...
static t_int *ripbob_perform(t_int *w)
{
    t_ripbob *x = (t_ripbob *)(w[1]);
...
static void ripbob_dsp(t_ripbob *x, t_signal **sp)
{
    x->x_sr = sp[0]->s_sr;
    dsp_add(ripbob_perform, 6, x, sp[0]->s_vec, sp[1]->s_vec,
...
void ripbob_tilde_setup(void)
{
    ...
    class_addmethod(ripbob_class, (t_method)ripbob_saturation, gensym("saturation"),
        A_FLOAT, 0);
    class_addmethod(ripbob_class, (t_method)ripbob_oversample, gensym("oversample"),
        A_FLOAT, 0);
    class_addmethod(ripbob_class, (t_method)ripbob_clear, gensym("clear"), 0);
    class_addmethod(ripbob_class, (t_method)ripbob_print, gensym("print"), 0);
    class_addmethod(ripbob_class, (t_method)ripbob_error, gensym("error"), 0);

    class_addmethod(ripbob_class, (t_method)ripbob_dsp, gensym("dsp"), A_CANT, 0);
    CLASS_MAINSIGNALIN(ripbob_class, t_ripbob, x_f);
}
