...
typedef struct _moop {
    ...
    double x_trns; //held transposition
    ...
    double x_tperiod; //period without samplerate
...
static void moop_tempo(t_moop *x, t_floatarg tempo) {
    x->x_tempo = (tempo > 0.f) ? tempo : 1.f;
}

static void moop_time(t_moop *x, t_symbol* SHADYLIB_UNUSED(s), int argc, t_atom *argv) {
    t_float time;
    if(argc > 0) {
        if(argv[0].a_type == A_FLOAT) {
            ...
            if(time == 0.f) {
                ...
                x->x_tperiod = 1000.0/(time*(x->x_tempo));
        ...
        if(argc > 1)
            if(argv[1].a_type == A_FLOAT) {
                ...
                x->x_inf = 0;
                x->x_phase = 0.0;
                if(time < 0.f) x->x_sample = x->x_range;
                else x->x_sample = 0.0;
                if(num <= 0) {
                    x->x_num = 0;
                    x->x_phase = 0.0;
...
static void moop_period(t_moop *x, t_floatarg period) {
    x->x_tperiod = 1000.0/(period*(x->x_tempo));
    x->x_period = x->x_tperiod/x->x_sr;
}
